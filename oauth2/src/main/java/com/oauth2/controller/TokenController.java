    ...
    public Object token(HttpServletRequest request) throws URISyntaxException, OAuthSystemException {
        ...
        try {
            ...
            if (oauthRequest.getParam(OAuth.OAUTH_GRANT_TYPE).equals(GrantType.AUTHORIZATION_CODE.toString())) {
            	...
            	String app_account_id = redisUtils.get(authCode);
                ...
                // 传递state，并删除Redis中缓存
                String state = redisUtils.get(Constants.STATE_IN_REDIS_PERFIX + authCode);
                redisUtils.remove(Constants.STATE_IN_REDIS_PERFIX + authCode);
                ...
                params.put("CLIENT_ID", oauthRequest.getClientId());
                redisUtils.getStringRedisTemplate().opsForValue().set(refreshToken, state, Constants.REFRESH_TOKEN_EXPIREIN, TimeUnit.SECONDS);
                //生成access Token
            ...
            if (oauthRequest.getParam(OAuth.OAUTH_GRANT_TYPE).equals(GrantType.REFRESH_TOKEN.toString())) {
            	...
            	String state = redisUtils.getStringRedisTemplate().opsForValue().get(refresh_token);
            	//生成refresh Token
                OAuthIssuer oauthIssuerImpl = new OAuthIssuerImpl(new MD5Generator());
                refreshToken = oauthIssuerImpl.refreshToken();
                Map<String, Object> map = new HashMap<String, Object>();
