    ...
    public Object authorize(HttpServletRequest request)throws URISyntaxException, OAuthSystemException {
        try {
            ...
            if (responseType.equals(ResponseType.CODE.toString())) {
                ...
                // 将Code 缓存到 Redis中，有效时长为5分钟
                //redisUtils.setex(authorizationCode, app_account_id);
                redisUtils.setex(authorizationCode, request.getParameter("username"));
            }
            //进行OAuth响应构建
            ...
            redisUtils.setex(Constants.STATE_IN_REDIS_PERFIX + authorizationCode, state);
            //得到到客户端重定向地址
